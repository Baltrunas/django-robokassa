...
from robokassa.conf import STRICT_CHECK, FORM_TARGET, CURRENCY, EXTRA_PARAMS
...
class RobokassaForm(BaseRobokassaForm):
    ...
    if CURRENCY:
        OutSumCurrency = forms.CharField(max_length=10, initial=CURRENCY)
    ...
    def _get_signature_string(self):
        ...
        if CURRENCY:
            standard_part = ':'.join([_val('MrchLogin'), _val('OutSum'), _val('InvId'), CURRENCY, PASSWORD1])
        else:
